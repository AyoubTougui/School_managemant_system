        ...
        @foreach ($parents->children as $key => $children)
            <div class="w-full bg-gray-200 text-center border border-gray-300 rounded px-8 py-6 mb-4 {{ ($key>=1) ? 'ml-0 sm:ml-2' : '' }} {{ ($parents->children_count===1) ? 'sm:max-w-sm' : '' }}">
                <div class="text-gray-700 font-bold">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="w-1/2 text-sm text-right">Group :</div>
                        <div class="w-1/2 text-sm text-left ml-2">{{ $children->group->name }}</div>
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('attendance.show',['student'=>$children->user->id]) }}" class="bg-gray-100 inline-block mb-4 text-xs text-gray-600 uppercase font-semibold px-4 py-2 border border-gray-400 rounded">Attendence</a>
                        <a href="{{ route('planning.DisplayPlanning',['child' => $children->id]) }}" class="bg-gray-100 inline-block mb-4 text-xs text-gray-600 uppercase font-semibold px-4 py-2 border border-gray-400 rounded">Planning</a>
                    </div>
                </div>
            </div>
        @endforeach
