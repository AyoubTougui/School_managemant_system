...
@section('content')
    <div class="roles">
        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('exam.update',$exam->id) }}" method="POST" class="w-full max-w-xl px-6 py-12">
                @csrf
                @method('PUT')
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="name" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" value="{{$exam->name}}">
                        @error('name')
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="date" id="datepicker-tc" autocomplete="off" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" value="{{$exam->date}}">
                        @error('dateofbirth')
        ...
        $('.dynamic').change(function () {
               ...
               $.ajax({
                   ...
                   success:function(data){
                       ...
                       {
                           txt+='<option value="'+group[i].id+'"'+(<?php echo $exam->class_id;?> == group[i].id ?  "selected" : "")+'>'+group[i].name+'</option>';
                       }
                       $('.group_id').html(txt);
                       ...
                       {
                           txt2+='<option value="'+subject[i].id+'"'+(<?php echo $exam->subject_id;?> == subject[i].id ?  "selected" : "")+'>'+subject[i].name+'</option>';
                       }
