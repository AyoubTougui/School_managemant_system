...
<div class="flex items-center justify-between mb-6"> 
    <div>
        <h2 class="text-gray-700 uppercase font-bold">Update Planning</h2>
    </div>
...
<div class="table w-full mt-8 bg-white rounded">
    <form action="{{ route('planning.update',$plannings->id) }}" method="POST" class="w-full max-w-xl px-6 py-12">
        @csrf
        @method('PUT')
            ...
            <div class="md:w-2/3">
                    ...
                    >
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ $plannings->subject_id == $subject->id ? 'selected' :'' }}>{{ $subject->name}}</option>
                        @endforeach
                    </select>
                @error('name')
            ...
            <div class="md:w-2/3">
                    <select name="teacher_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 " id="grid-state">
                        ...
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ $plannings->teacher_id == $teacher->id ? 'selected' :'' }}>{{ $teacher->name}}</option>
                        @endforeach
                    </select>
                @error('name')
            ...
            <div class="md:w-2/3">
                    <select name="room_id" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                        ...
                        @foreach ($rooms as $room)
                            <option value="{{ $room->id }}" {{ $plannings->room_id == $room->id ? 'selected' :'' }}>{{ $room->name}}</option>
                        @endforeach
                    </select>
                @error('name')
